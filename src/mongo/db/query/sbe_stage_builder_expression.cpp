...
#include "mongo/db/query/sbe_compile_time_parser.h"
...
public:
    ...
    void visit(ExpressionArrayElemAt* expr) final {
        using namespace mongo::sbe_ctp;
        _context->ensureArity(2);

        auto index = _context->popExpr();
        auto array = _context->popExpr();
        auto placeholderList = sbe::makeEs(std::move(array), std::move(index));

        constexpr auto code = "getElement({0}, {1})"_sbe;

        auto resultExpr = code.build(placeholderList);
